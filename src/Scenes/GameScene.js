...
export default class GameScene extends Phaser.Scene {
  ...
  sendFire() {
    if (this.power > 0 && this.fires.countActive(true) === 0) {
      const fire = this.fires.create(this.player.x, this.player.y, 'fire');
      fire.setScale(0.25);
  ...
  create() {
    ...
    const backgrounds = this.physics.add.staticGroup({
      key: 'background',
      repeat: 10,
    ...
    const badies = this.physics.add.group({
      key: 'bady',
      repeat: 10,
